...
public class FavProvider extends ContentProvider {
     ...
     */
    private static final UriMatcher favUriMatcher = new UriMatcher(UriMatcher.NO_MATCH);
    ...
    static {
        ...
        favUriMatcher.addURI(FavContract.CONTENT_AUTHORITY, FavContract.PATH_FAVORITES, FAVORITES);

        // The content URI of the form "content://com.virtual.warehouse/products/#" will map to the
        // integer code {@link #FAVORITE_ID}. This URI is used to provide access to ONE single row
        // of the products table.
        //
        ...
        favUriMatcher.addURI(FavContract.CONTENT_AUTHORITY, FavContract.PATH_FAVORITES + "/#", FAVORITE_ID);
    }

    /** Database helper object */
    private FavDbHelper mDbHelper;
    ...
    public Cursor query(Uri uri, String[] projection, String selection, String[] selectionArgs,
        ...
        switch (match) {
            ...
            case FAVORITES:
                cursor = database.query(FavEntry.TABLE_NAME, projection, selection, selectionArgs,
                        null, null, sortOrder);
                break;
            case FAVORITE_ID:

                selection = FavEntry._ID + "=?";
    ...
    private Uri insertMovie(Uri uri, ContentValues values) {
        ...
        int movie_id = values.getAsInteger(FavEntry.TMDB_MOVIE_ID);
        if (movie_id == 0) {
            throw new IllegalArgumentException("Favorite requires an id");
        }

        // Check that the name is not null
        String name = values.getAsString(FavEntry.COLUMN_MOVIE_NAME);
        if (name == null) {
        ...
        long id = database.insert(FavEntry.TABLE_NAME, null, values);
    ...
    private int updateFavorite(Uri uri, ContentValues values, String selection, String[] selectionArgs) {
        ...
        // If the {@link ProductEntry#COLUMN_MOVIE_NAME} key is present,
        // check that the name value is not null.
        if (values.containsKey(FavEntry.COLUMN_MOVIE_NAME)) {
            String name = values.getAsString(FavEntry.COLUMN_MOVIE_NAME);
            if (name == null) {
                throw new IllegalArgumentException("Favorite requires a name");
            }
        ...
        int rowsUpdated = database.update(FavEntry.TABLE_NAME, values, selection, selectionArgs);
    ...
    public int delete(Uri uri, String selection, String[] selectionArgs) {
        ...
        switch (match) {
            case FAVORITES:
                ...
                rowsDeleted = database.delete(FavEntry.TABLE_NAME, selection, selectionArgs);
                break;
            case FAVORITE_ID:
